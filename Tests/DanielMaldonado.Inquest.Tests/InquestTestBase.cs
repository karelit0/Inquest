...
namespace DanielMaldonado.Inquest.Tests
{
    public abstract class InquestTestBase : AbpIntegratedTestBase<InquestTestModule>
    {
        ...
        protected void UsingDbContext(Action<InquestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<InquestDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<InquestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<InquestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<InquestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<InquestDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<InquestDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<InquestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<InquestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<InquestDbContext, Task<T>> func)
        {
            T result;
